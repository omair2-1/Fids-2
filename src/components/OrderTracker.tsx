...
export const OrderTracker: React.FC<OrderTrackerProps> = ({ currentOrder, onOpenChatWithQuery }) => {
  ...
  const fetchOrder = async (idToFetch: string) => {
    ...
    try {
      ...
      if (data.success && data.order) {
        ...
        setSearchError(`Order "${idToFetch}" not found. Try searching for ORD-9102 or ORD-9101.`);
      }
    } catch (err) {
  ...
  useEffect(() => {
    if (currentOrder) {
      ...
      // Default to first active order demo if available
      fetchOrder('ORD-9102');
    }
  }, [currentOrder]);

  // Simulate advancing status for testing live updates
  const advanceStatusDemo = async () => {
    if (!activeOrder) return;
    const statuses: OrderStatus[] = ['Pending', 'Preparing', 'Ready', 'Out for Delivery', 'Delivered'];
    ...
    try {
      const res = await fetch(`/api/orders/${activeOrder.id}/status`, {
        ...
        body: JSON.stringify({
          status: nextStatus,
          note: `FIDS Kitchen updated status to ${nextStatus}`
        })
      });
      const data = await res.json();
      if (data.success && data.order) {
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <div className="relative flex-1 sm:w-64">
              ...
              <input
                type="text"
                placeholder="Enter Order ID (e.g. ORD-9102)"
                value={searchId}
        ...
        <div className="bg-stone-900 rounded-3xl border border-stone-800 shadow-2xl overflow-hidden">
          ...
          <div className="p-4 bg-amber-500/10 border-b border-amber-500/20 flex flex-wrap items-center justify-between gap-3 text-xs">
            <div className="flex items-center gap-2 text-amber-300 font-bold">
              ...
              <span>Interactive Demo: Advance Kitchen Status</span>
            </div>
