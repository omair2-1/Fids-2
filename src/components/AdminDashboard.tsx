...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    if (passkeyInput === 'admin123' || passkeyInput === 'admin' || passkeyInput === '') {
      setIsAuthenticated(true);
      setAuthError('');
    } else {
      setAuthError('Invalid Admin Passkey. Try "admin123" or click Quick Access.');
    }
  };
    ...
    return (
      <div className="max-w-md mx-auto my-12 bg-white rounded-3xl p-8 border border-stone-200 shadow-xl text-stone-900 space-y-6">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <button
            type="submit"
            className="w-full py-3.5 rounded-xl bg-amber-600 text-white font-bold text-sm hover:bg-amber-500 shadow-lg shadow-amber-600/20 transition-colors"
          >
        ...
        <div className="pt-4 border-t border-stone-100 text-center">
          ...
          >
            ⚡ Quick Passkey Bypass (Demo Staff Access)
          </button>
        </div>
      </div>
    );
