...
import { X, Trash2, Plus, Minus, CreditCard, ShoppingBag, ArrowRight, CheckCircle2, ShieldCheck, MapPin, Phone, User, Utensils, AlertCircle, Sparkles } from 'lucide-react';
...
export const CartDrawer: React.FC<CartDrawerProps> = ({
  ...
  const handleCheckoutSubmit = (e: React.FormEvent) => {
    ...
    if (paymentMethod === 'Cash on Delivery') {
      // No real payment needed — just a quick confirm step.
  ...
  // Opens Razorpay's own hosted checkout (real UPI/GPay/PhonePe/card payment).
  // The order only gets created in processFinalOrder AFTER a real, verified
  ...
  const startRazorpayCheckout = async () => {
    ...
    try {
      ...
      rzp.on('payment.failed', () => {
        setErrorMessage('Payment failed or was declined. Please try again.');
        setIsSubmitting(false);
      });
  ...
  const processFinalOrder = async (razorpayVerification?: {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden bg-black/80 backdrop-blur-md flex justify-end">
      ...
      {/* Cash on Delivery Confirmation Modal (online payments go through Razorpay's own hosted checkout instead) */}
      {showPaymentModal && (
          ...
          <div className="bg-stone-900 rounded-3xl max-w-sm w-full p-6 space-y-5 shadow-2xl border border-amber-500/40 text-stone-100 relative">
            <div className="flex justify-between items-center border-b border-stone-800 pb-3">
              <div className="flex items-center gap-2">
                ...
                <span className="font-extrabold text-sm text-white">Confirm Cash on Delivery</span>
              </div>
            ...
            <div className="bg-stone-950 p-4 rounded-2xl border border-stone-800 text-center space-y-2">
              <span className="text-xs text-stone-400 font-medium">Amount Payable to Rider/Staff</span>
